...
async function startBot() {
  ...
  // 2. Hedefe (MAX_REPLIES) ulaşana kadar turlarla çalış
  while (repliesSent < MAX_REPLIES) {
    console.log(`\n====================================================`);
    console.log(`🔄 TUR ${round} BAŞLIYOR (İlerleme: ${repliesSent}/${MAX_REPLIES} Yanıt)`);
    console.log(`====================================================`);
    ...
    for (const rawQuery of config.searchQueries) {
      ...
      // Sayfayı aşağı kaydırarak son 24 saatteki daha fazla tweet'i yükle
      for (let s = 0; s < 3; s++) {
    ...
    // Bir tur bittiğinde hedefe (MAX_REPLIES) henüz ulaşılamadıysa
    const roundWaitMinutes = 5;
    console.log(`\n====================================================`);
    console.log(`🏁 ${round}. Tur tamamlandı. Bu turda ${newRepliesThisRound} yeni yanıt gönderildi.`);
    console.log(`📊 Toplam İlerleme: ${repliesSent} / ${MAX_REPLIES} Yanıt`);
    console.log(`⏳ X'te yeni tweetlerin paylaşılması için ${roundWaitMinutes} dakika bekleniyor... (Durdurmak için Ctrl+C yapabilirsiniz)`);
    console.log(`====================================================`);

    for (let m = roundWaitMinutes; m > 0; m--) {
      console.log(`⏱️ Sonraki tura (${round + 1}. Tur) ${m} dakika kaldı...`);
      await sleep(60000);
    }
    round++;
  }
  ...
  console.log("\n====================================================");
  console.log(`🎯 Hedefe Ulaşıldı! Oturum Tamamlandı. Toplam atılan yanıt: ${repliesSent}`);
  console.log("====================================================");
  await context.close();
}
